...
static message_container_t& message() {
    static string message_string [8][5][4]= 
        {{{"encstrset_new(", ")", "", ""},
...
bool general_operation(request_type type, unsigned long id, const char *value, const char *key) {
    ...
    string key_str = key == NULL ? "" : key;
    diagnostic_message(type, intro, to_string(id),
                       (value == NULL ? "NULL" : quote(value_str)),
                       (key == NULL ? "NULL" : quote(key_str)));
