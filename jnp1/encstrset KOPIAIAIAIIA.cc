...
namespace {  // funkcje pomocnicze
    ...
    const string message[OP_COUNT][MESSAGE_TYPE_COUNT][MESSAGE_SEGMENT_COUNT] =
            {{{"encstrset_new(",    ")",  "",   ""},
    ...
    static unordered_map<unsigned long, unordered_set<string>> &encstrset_map() {
    ...
    string to_hex(string src) {  // zwraca src zapisany w systemie szesnastkowym wielkimi literami
        ...
        for (size_t i = 0; i < size; i++) {
            if (src[i] == '\0') // FIXME: dwie cyfry zamiast jednej
                ss << "00";
            else
